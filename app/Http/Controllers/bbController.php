...
class bbController extends Controller
{
    public function input_data(Request $request)
    {
        ...
        $newData = new bbtables();
        $newData->keterangan = $request->keterangan_form;
        $newData->file = $filePath;
        $newData->save();

        return redirect('/bb')->with('success', 'Data berhasil disimpan');
    }
    ...
    public function update_data(Request $request)
    {
        ...
        $dataToUpdate = bbtables::find($request->id);
        if ($dataToUpdate) {
            ...
            return redirect('/bb')->with('success', 'Data berhasil diperbarui');
        } else {
            return redirect('/bb')->with('error', 'Data tidak ditemukan');
        }
    }

    public function del_data($id)
    {
        $dataToDelete = bbtables::find($id);
        if ($dataToDelete) {
            ...
            return redirect('/bb')->with('success', 'Data berhasil dihapus');
        } else {
            return redirect('/bb')->with('error', 'Data tidak ditemukan');
        }
    }
