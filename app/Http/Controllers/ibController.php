...
class ibController extends Controller
{
    public function input_data(Request $request)
    {
        ...
        $newData = new ibtables();
        $newData->keterangan = $request->keterangan_form;
        $newData->file = $filePath;
        $newData->save();

        return redirect('/ib')->with('success', 'Data berhasil disimpan');
    }
    ...
    public function update_data(Request $request)
    {
        ...
        $dataToUpdate = ibtables::find($request->id);
        if ($dataToUpdate) {
            ...
            return redirect('/ib')->with('success', 'Data berhasil diperbarui');
        } else {
            return redirect('/ib')->with('error', 'Data tidak ditemukan');
        }
    }

    public function del_data($id)
    {
        $dataToDelete = ibtables::find($id);
        if ($dataToDelete) {
            ...
            return redirect('/ib')->with('success', 'Data berhasil dihapus');
        } else {
            return redirect('/ib')->with('error', 'Data tidak ditemukan');
        }
    }
