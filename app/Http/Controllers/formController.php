...
class formController extends Controller
{
    ...
    public function update_data(Request $request)
    {
        ...
        $dataToUpdate = formtables::find($request->id);
        if ($dataToUpdate) {
            ...
            return redirect('/form')->with('success', 'Data berhasil diperbarui');
        } else {
            return redirect('/form')->with('error', 'Data tidak ditemukan');
        }
    }

    public function del_data($id)
    {
        $dataToDelete = formtables::find($id);
        if ($dataToDelete) {
