...
class aiController extends Controller
{
    public function input_data(Request $request)
    {
        ...
        $newData = new aitables();
        $newData->keterangan = $request->keterangan_form;
        $newData->file = $filePath;
        $newData->save();

        return redirect('/ai')->with('success', 'Data berhasil disimpan');
    }
    ...
    public function update_data(Request $request)
    {
        ...
        $dataToUpdate = aitables::find($request->id);
        if ($dataToUpdate) {
            ...
            return redirect('/ai')->with('success', 'Data berhasil diperbarui');
        } else {
            return redirect('/ai')->with('error', 'Data tidak ditemukan');
        }
    }

    public function del_data($id)
    {
        $dataToDelete = aitables::find($id);
        if ($dataToDelete) {
            ...
            return redirect('/ai')->with('success', 'Data berhasil dihapus');
        } else {
            return redirect('/ai')->with('error', 'Data tidak ditemukan');
        }
    }
