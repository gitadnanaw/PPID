...
class spController extends Controller
{
    public function input_data(Request $request)
    {
        ...
        $newData = new sptables();
        $newData->keterangan = $request->keterangan_form;
        $newData->file = $filePath;
        $newData->save();

        return redirect('/sp')->with('success', 'Data berhasil disimpan');
    }
    ...
    public function update_data(Request $request)
    {
        ...
        $dataToUpdate = sptables::find($request->id);
        if ($dataToUpdate) {
            ...
            return redirect('/sp')->with('success', 'Data berhasil diperbarui');
        } else {
            return redirect('/sp')->with('error', 'Data tidak ditemukan');
        }
    }

    public function del_data($id)
    {
        $dataToDelete = sptables::find($id);
        if ($dataToDelete) {
